...
whole reason the two telemetry vendors are two modules,
`pelican-client-java` asserts it carries no HTTP library beyond the
JDK's, and `pelican-test` asserts it drags in no server library and no matcher
