...
- **Item 3** shipped its first phase: `ClientTransport` in core and
  `pelican-client-java` over the JDK's own `HttpClient`. The two things that
  phase was supposed to make cheap have since landed, and were:
  ...
  The Ktor and Pekko adapters are still to do, and are the last of this item.
