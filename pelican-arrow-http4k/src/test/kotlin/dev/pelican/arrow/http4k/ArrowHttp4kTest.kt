...
import dev.pelican.test.http4k.inMemoryHttp4k
...
/**
 * The Arrow binders over a real http4k handler.
 *
 ...
 * The requests run through the interpreted handler rather than a socket, which is
 * every layer that turns an Outcome into a response.
 */
class ArrowHttp4kTest {
    ...
    private fun app() = Api(
        endpoints = listOf(
            ...
            streamItems.streamedRaise(toResponse) { max ->
                ...
                (1..max).map { Item(it.toLong(), "item-$it") }.asSequence()
            },
            watchItems.streamedByRaise(toResponse) { max ->
                if (max <= 0) raise(Problem.Empty)
                CompletableFuture.completedStage((1..max).map { Item(it.toLong(), "item-$it") }.asSequence())
            },
        ),
